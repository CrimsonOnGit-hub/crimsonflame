...
function loadFriends() {
    onSnapshot(doc(db, "users", currentUser.uid), (docSnap) => {
        ...
        data.friends.forEach(friend => {
            ...
            el.innerHTML = `<img src="${DEFAULT_PFP}" style="width:24px; border-radius:50%;"> ${friend.displayName}`;
...
function loadChats() {
    onSnapshot(query(collection(db, "dialog_chats"), where("participants", "array-contains", currentUser.uid)), snap => {
        ...
        snap.forEach(docSnap => {
            ...
            el.innerHTML = `<img src="${DEFAULT_PFP}" style="width:24px; border-radius:50%;"> ${chatName}`;
...
function openChat(chatId, data, element, nameOverride) {
    ...
    messagesUnsub = onSnapshot(query(collection(db, "dialog_chats", chatId, "messages"), orderBy("timestamp", "asc")), snap => {
        ...
        snap.forEach(mDoc => {
            ...
            messagesBox.innerHTML += `
                <div class="dc-msg">
                    <img src="${DEFAULT_PFP}" class="dc-avatar">
                    <div class="dc-msg-content">
                        <div class="dc-msg-header"><span class="dc-username">${m.senderName}</span> <span class="dc-timestamp">${formatDiscordTime(m.timestamp)}</span></div>
...
messageForm.onsubmit = async (e) => {
    ...
    await addDoc(collection(db, "dialog_chats", activeChatId, "messages"), { text: messageInput.value.trim(), senderUid: currentUser.uid, senderName: myUsername, timestamp: serverTimestamp() });
    messageInput.value = "";
};
